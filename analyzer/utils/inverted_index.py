...
class DataPolisher:
    """WRONG --- Class that encapsultes the inverted index to specify query in a system
    Provide proper tf-idf scaling and everything is kept in memory (the dataset is really small).
    Stemming is applied.
    """
    ...
    def clean_supporter(self):
        """Same as prepare dataset, but returns a list insted of dict and doesn't save anything"""
    ...
    def clean_text(self, data_path='data/cran.all.1400',algorithm='w2v'):
        """Read only titles and abstract body oh each documnt.
        Builds and save a list of iterable depending on the algorithm
        chosen (w2v or else)
        In realtà unserò word2vec per tutto.
        """
        if algorithm == 'w2v':
...
class InvertedIndex:
    ...
    def query(self, query ,k=10):
        ...
        #Compute it against all documents tf-idf vectors and take
        #the top k
        scores = []
        for doc in self.vsm:
